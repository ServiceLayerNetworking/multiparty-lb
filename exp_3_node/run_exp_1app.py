...
def run_hit(q, variation, proc_distr, app_nums, rpses, distr):
    ...
    for i, app_num in enumerate(app_nums):
        ...
        rps = rpses[i]
        req_interval_ms = (1000.0 / float(rps)) if rps != 0 else float(DURATION+1)*1000.0
        ...
        for nodes_app2 in itertools.combinations(nodes, 2):
            for nodes_app3 in itertools.combinations(nodes, 1):
                ...
                if run_id in [7]:
                    ...
                    for lb in ["leastrequest"]: #"locality_aware_weighted_random"]:
                            ...
                            for iteration in [1, 2, 3]:
                                ...
                                for distr in ["exponential"]:
                                    ...
                                    for proc_distr in ["exponential"]:
                                        ...
                                        # input()
                                        run_exp(f"{distr}_{proc_distr}_c_mplb_{LB_NAME[lb]}_{run_id}_{rps}rps_{iteration}", rpses, "LB", distr, proc_distr, append_to_times=to_append)
